			...
			Commands: Commands{
				"list": {
					...
					Exec: func(user auth.User, args ...string) (con []byte, typ string, code int, err error) {
						hashes, err := c.hooks.Auth.ListUsers()
						if err != nil {
				...
				"get": {
					...
					ArgsLen:         [2]int{1, 1},
					Exec: func(user auth.User, args ...string) (con []byte, typ string, code int, err error) {
						userData, err := c.hooks.Auth.GetUser(args[0])
						if err != nil {
							return []byte{}, "", http.StatusBadRequest, err
						}
				...
				"create": {
					...
					Exec: func(user auth.User, args ...string) (con []byte, typ string, code int, err error) {
						...
						hash, err := c.hooks.Auth.CreateUser(auth.User{
							UID:      "",
						...
						userData, err := c.hooks.Auth.GetUser(hash)
						if err != nil {
							return []byte{}, "", http.StatusBadRequest, err
						}
						jsonBytes, err := json.Marshal(userData)
				...
				"modify": {
					...
					Exec: func(user auth.User, args ...string) (con []byte, typ string, code int, err error) {
						...
						userHash, err := c.hooks.Auth.ModifyUser(args[0], userData)
						if err != nil {
							return []byte{}, "", http.StatusBadRequest, err
						}
						userData, err = c.hooks.Auth.GetUser(userHash)
						if err != nil {
							return []byte{}, "", http.StatusBadRequest, err
						}
						jsonBytes, err := json.Marshal(userData)
				...
				"delete": {
					...
					Exec: func(user auth.User, args ...string) (con []byte, typ string, code int, err error) {
						...
						if err := c.hooks.Auth.DeleteUser(args[0]); err != nil {
							return []byte{}, "", http.StatusBadRequest, err
						}
				...
				"deauth": {
					...
					Commands: Commands{
						"user": {
							...
							ArgsLen:         [2]int{1, 1},
							Exec: func(user auth.User, args ...string) (con []byte, typ string, code int, err error) {
								c.hooks.Auth.Deauthenticate(args[0])
								return []byte(args[0]), TypeTXT, http.StatusOK, nil
							},
						},
						"token": {
							...
							ArgsLen:         [2]int{1, 1},
							Exec: func(user auth.User, args ...string) (con []byte, typ string, code int, err error) {
								c.hooks.Auth.DeauthenticateToken(args[0])
								return []byte(args[0]), TypeTXT, http.StatusOK, nil
							},
						},
					},
		...
		"tokens": {
			...
			Commands: Commands{
				...
				"get": {
					...
					ArgsLen:         [2]int{1, 1},
					Exec: func(user auth.User, args ...string) (con []byte, typ string, code int, err error) {
						data, err := c.hooks.Auth.GetToken(args[0])
						if err != nil {
							return []byte{}, "", http.StatusBadRequest, err
						}
						jsonBytes, err := json.Marshal(data)
